...
DOCUMENTATION = """
---
module: cckm_gcp_synchronization_job
short_description: Start and cancel CCKM Google Cloud synchronisation jobs
description:
    - Starts a job that synchronises Cloud KMS keys into CCKM, or cancels a running one.
    - The job runs asynchronously. Poll it with
      M(thalesgroup.ciphertrust.cckm_gcp_synchronization_job_info).
version_added: "1.1.0"
author:
  - Anurag Jain (@anugram)
...
options:
    ...
    key_rings:
      description:
        - Key rings to synchronise. Ignored when I(synchronize_all=true).
      type: list
      elements: str
    synchronize_all:
      description:
        - Synchronise every key ring CCKM manages.
      type: bool
"""

EXAMPLES = """
- name: "Synchronise one key ring"
  thalesgroup.ciphertrust.cckm_gcp_synchronization_job:
...
- name: "Synchronise every key ring"
  thalesgroup.ciphertrust.cckm_gcp_synchronization_job:
    localNode: "{{ cm_connection }}"
    op_type: start
...
argument_spec = dict(
    ...
    key_rings=dict(type="list", elements="str", no_log=False),
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "start":
            ...
            result["response"] = cckm_gcp.sync_start(
                node=node,
            ...
            check_mode_action(module)
            result["response"] = cckm_gcp.sync_cancel(
                node=node,
