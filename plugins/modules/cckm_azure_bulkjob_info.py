...
DOCUMENTATION = """
---
module: cckm_azure_bulkjob_info
short_description: Read CCKM Azure bulk jobs
description:
    - Lists or reads CCKM Azure bulk jobs.
    - Start or cancel one with M(thalesgroup.ciphertrust.cckm_azure_bulkjob).
    - A bulk job runs asynchronously. Poll C(get) until C(overall_status) leaves
      C(in_progress).
version_added: "1.1.0"
author:
  - Anurag Jain (@anugram)
...
EXAMPLES = """
- name: "Wait for a bulk job to finish"
  thalesgroup.ciphertrust.cckm_azure_bulkjob_info:
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "list":
            result["response"] = cckm_azure.bulkjob_list(
                node=node,
            ...
            result["response"] = cckm_azure.bulkjob_get(
                node=node,
