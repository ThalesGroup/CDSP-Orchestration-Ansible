...
DOCUMENTATION = """
---
module: cckm_azure_synchronization_job_info
short_description: Read CCKM Azure synchronisation jobs
description:
    - Lists or reads the jobs that synchronise Azure keys, certificates or secrets into
      CCKM.
    - Each kind of vault object has its own synchronisation service, chosen with
      I(scope).
    - Start or cancel a job with
      M(thalesgroup.ciphertrust.cckm_azure_synchronization_job).
version_added: "1.1.0"
author:
  - Anurag Jain (@anugram)
...
options:
    ...
    scope:
      description:
        - Which synchronisation service to read.
...
EXAMPLES = """
- name: "Wait for a key synchronisation job to finish"
  thalesgroup.ciphertrust.cckm_azure_synchronization_job_info:
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "list":
            result["response"] = cckm_azure.sync_list(
                node=node,
            ...
            result["response"] = cckm_azure.sync_get(
                node=node,
                scope=module.params.get("scope"),
