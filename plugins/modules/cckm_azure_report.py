...
DOCUMENTATION = """
---
module: cckm_azure_report
short_description: Start and delete CCKM Azure reports
description:
    - Starts a CCKM Azure report, or deletes a finished one.
    - A report runs asynchronously. Poll it with
      M(thalesgroup.ciphertrust.cckm_azure_report_info) until C(overall_status) leaves
...
options:
    ...
    log_analytic_params:
      description:
        - Azure Log Analytics workspaces the report reads from.
        - Required for C(create).
      type: list
      elements: dict
    start_time:
...
EXAMPLES = """
- name: "Start a key report"
  thalesgroup.ciphertrust.cckm_azure_report:
    localNode: "{{ cm_connection }}"
    op_type: create
    name: quarterly-keys
    report_type: key-report
    log_analytic_params:
      - workspace_id: "{{ azure_log_workspace }}"
  register: _report
"""
...
argument_spec = dict(
    ...
    log_analytic_params=dict(type="list", elements="dict"),
...
def setup_module_object():
    module = ThalesCipherTrustModule(
        ...
        required_if=(
            ["op_type", "create", ["name", "report_type", "log_analytic_params"]],
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "create":
            ...
            result["response"] = cckm_azure.report_create(
                node=node,
                name=params.get("name"),
                report_type=params.get("report_type"),
                log_analytic_params=params.get("log_analytic_params"),
            ...
            check_mode_action(module)
            result["response"] = cckm_azure.report_delete(
                node=node,
