...
DOCUMENTATION = """
---
module: cckm_oci_report_info
short_description: Read CCKM OCI reports and their contents
description:
    - Lists or reads CCKM OCI reports, and fetches a finished report's contents.
    - Start a report with M(thalesgroup.ciphertrust.cckm_oci_report).
...
- name: "Wait for a report to finish, then read it"
  thalesgroup.ciphertrust.cckm_oci_report_info:
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "list":
            result["response"] = cckm_oci.report_list(
                node=node,
            ...
            result["response"] = cckm_oci.report_get(
                node=node,
                report_id=module.params.get("report_id"),
            )
        elif op_type == "contents":
            result["response"] = cckm_oci.report_contents(
                node=node,
                report_id=module.params.get("report_id"),
            )
        elif op_type == "download":
            result["response"] = cckm_oci.report_download(
                node=node,
                report_id=module.params.get("report_id"),
            )
        else:
