...
DOCUMENTATION = """
---
module: cckm_gcp_synchronization_job_info
short_description: Read CCKM Google Cloud synchronisation jobs
description:
    - Lists or reads the jobs that synchronise Cloud KMS keys into CCKM.
    - Start or cancel a job with
      M(thalesgroup.ciphertrust.cckm_gcp_synchronization_job).
...
- name: "Wait for a synchronisation job to finish"
  thalesgroup.ciphertrust.cckm_gcp_synchronization_job_info:
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "list":
            result["response"] = cckm_gcp.sync_list(
                node=node,
            ...
            result["response"] = cckm_gcp.sync_get(
                node=node,
