...
DOCUMENTATION = """
---
module: cckm_oci_report
short_description: Start and delete CCKM OCI reports
description:
    - Starts a CCKM OCI report, or deletes a finished one.
    - A report runs asynchronously. Poll it with
      M(thalesgroup.ciphertrust.cckm_oci_report_info) until C(overall_status) leaves
...
options:
    ...
    report_type:
      description:
        ...
        - Required for C(create).
      choices: [key-report, key-rotation, key-aging]
      type: str
    oci_params:
      description:
        - Which tenancies, compartments or vaults the report covers.
        - Required for C(create).
      type: list
      elements: dict
    start_time:
...
EXAMPLES = """
- name: "Start a key rotation report"
  thalesgroup.ciphertrust.cckm_oci_report:
...
argument_spec = dict(
    ...
    name=dict(type="str"),
    report_type=dict(type="str", choices=["key-report", "key-rotation", "key-aging"]),
    oci_params=dict(type="list", elements="dict"),
...
def setup_module_object():
    module = ThalesCipherTrustModule(
        ...
        required_if=(
            ["op_type", "create", ["name", "report_type", "oci_params"]],
...
def main():
    ...
    with ciphertrust_operation(module):
        if op_type == "create":
            ...
            result["response"] = cckm_oci.report_create(
                node=node,
            ...
            check_mode_action(module)
            result["response"] = cckm_oci.report_delete(
                node=node,
