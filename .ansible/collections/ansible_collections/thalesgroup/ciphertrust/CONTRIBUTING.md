...
The unit suite needs no CipherTrust Manager and gates on coverage:

```
pytest tests/unit
```

`ansible-test` must be run from a path that ends in
`ansible_collections/thalesgroup/ciphertrust`, so install the collection to
that layout first and run from there:

```
ansible-galaxy collection install . --force -p ~/.ansible/collections
cd ~/.ansible/collections/ansible_collections/thalesgroup/ciphertrust
ansible-test sanity
```

Certification runs sanity against the built artifact rather than the
repository, so also check `ansible-galaxy collection build` output the same
way -- `build_ignore` excludes files a waiver may not reference.

The integration suite talks to a real CipherTrust Manager. Copy the template
and fill in a reachable instance:

```
cp tests/integration/integration_config.yml.template \
   tests/integration/integration_config.yml
ansible-test integration --python 3.12 module_group_save
```

`integration_config.yml` holds live credentials and is deliberately
git-ignored and excluded from the build artifact. Never commit it.

Some targets are opt-in because they cannot be undone or restart the server;
each is guarded by a flag documented in the template.
