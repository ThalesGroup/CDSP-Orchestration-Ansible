...
notes:
  - >-
    ...
    user or client is already a member. When it can, repeating the task reports
    C(changed=false) and sends no request.
  - >-
    CipherTrust Manager 2.22 answers a GET on the membership path with 405, so
    on that release the module cannot establish membership and performs the add
    or remove, reporting C(changed) each time. This is deliberate: an
    inconclusive answer must never cause the operation to be skipped.
options:
    op_type:
        description:
