...
notes:
  - >-
    CipherTrust Manager addresses an interface by its name, which it derives
    from the type, network interface and port (for example C(nae_all_9005)).
    Pass that name as C(interface_id); the identifier returned as C(id) is not
    accepted on this API and yields a 404.
  - >-
    Creating an interface is not idempotent. CipherTrust Manager derives the
    interface name from its type, network interface and port (for example
    C(nae_all_9005)) and rejects a name supplied by the caller, so the module
    cannot look up an interface it created by the name given in the task.
    Running a create twice reports a port conflict rather than no change.
  - >-
